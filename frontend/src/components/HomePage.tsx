...
export function HomePage({ onNavigate }: HomePageProps) {
  ...
  // NEW: today's calories state
  const [todayCalories, setTodayCalories] = useState<number | null>(null);

  // NEW: load today's calories on mount
  useEffect(() => {
    const loadCalories = async () => {
      try {
        const today = new Date().toISOString().split('T')[0];
        const res = await fetch(`http://localhost:3000/api/calories/${today}`);
        const data = await res.json();
        setTodayCalories(data.total);
      } catch (err) {
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-purple-500 p-6 text-white">
        ...
        {/* NEW: Today’s Calories */}
        <div className="mt-2 bg-white/20 backdrop-blur-sm p-3 rounded-lg inline-block">
          <p className="text-xs text-purple-100">Today's Calories</p>
          <p className="text-xl font-semibold">
            {todayCalories !== null ? todayCalories : '...'} kcal
          </p>
        </div>
      </div>
